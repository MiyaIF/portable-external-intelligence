...
launch_agents_dir="${EI_LAUNCH_AGENTS_DIR:-$HOME/Library/LaunchAgents}"
plist="$launch_agents_dir/CodexExternalIntelligenceMaintenance-v1.plist"
mkdir -p "$launch_agents_dir"
"$python_exe" -B -c '
from pathlib import Path
import sys
from ei.config import load_settings
from ei.task_scheduler import build_launchd_plist, build_maintenance_action
engine, knowledge, home, runtime, python, output = map(Path, sys.argv[1:7])
...
Path(output).write_text(build_launchd_plist(action), encoding="utf-8", newline="\n")
' "$engine_root" "$knowledge_root" "$codex_home" "$runtime_root" "$python_exe" "$plist"

uid="$(id -u)"
label="CodexExternalIntelligenceMaintenance-v1"
...
printf '%s\n' "Registered user launch agent $label."
