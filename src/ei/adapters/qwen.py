...
class QwenAdapter:
    """Read stable Qwen session metadata records only."""

    parser_version = "qwen-code-session-metadata-v1"
    capture_path = "native_memory"
    host_id = "qwen-code"
    host_family = "qwen-compatible"
    ...
    def read(self, source: Path) -> Iterable[SourceRecord]:
        ...
        if document.get("host", "qwen-code") != "qwen-code":
            raise ValueError("UNSUPPORTED_FORMAT")
        sessions = document.get("sessions", document.get("records"))
        ...
        for session in sessions:
            ...
            if session_id is None:
                ...
                self.health["parse_skipped"] += 1
                continue
            entries = session.get("observations", session.get("entries"))
            ...
            for raw in entries:
                if not isinstance(raw, dict):
                    ...
                    self.health["parse_skipped"] += 1
                    continue
                if any(key in raw for key in ("prompt", "response", "messages", "transcript", "tool_output")):
                    self.health["excluded"] += 1
                    continue
                title = self._text(raw.get("title"), True)
                claim = self._text(raw.get("claim", raw.get("rule", raw.get("content"))), True)
                if title is None or claim is None:
                    self.parse_skipped += 1
                    self.health["parse_skipped"] += 1
                    continue
                domain = self._text(raw.get("domain")) or "general"
                outcome = self._text(raw.get("outcome_status")) or "unknown"
                benefit = self._text(raw.get("benefit")) or "reduced_search"
                entry_id = self._text(raw.get("id")) or hashlib.sha256((session_id + title).encode("utf-8")).hexdigest()[:20]
                cwd = self._text(raw.get("cwd_fingerprint", raw.get("scope_hash"))) or session_scope
                classification = self._text(raw.get("classification")) or "private-reusable"
